...
def build_report():
    ...
    # Create filtered view for Snapshot tables (hide closed positions)
    # Must do this AFTER asset_class_short is added to sec_only
    sec_current = sec_only[sec_only["shares"].abs() > 1e-6].copy()
    ...
    # Pre-load transactions & dividends for ticker PL to avoid re-loading inside loop
    tx_all_raw = load_transactions_raw()
    div_all_raw = load_dividends()
    ...
    if not sec_full.empty:
        # Build numeric version for selecting top/bottom performers (Filtered)
        sec_highlights = sec_full[sec_full["shares"].abs() > 1e-6].copy()
        sec_full_numeric = sec_highlights.copy()
        for col in ["1M", "1D"]:
    ...
    # Exclude CASH from weight calculations (Use sec_current)
    sec_no_cash = sec_current[sec_current["ticker"] != "CASH"].copy()
    ...
    # Compute prices (CASH = 1) on sec_current
    sec_current["price"] = sec_current["ticker"].map(lambda t: 1 if t == "CASH" else latest_prices[t])
    sec_current["value"] = sec_current["shares"] * sec_current["price"]
    total_value = sec_current["value"].sum()
    sec_current["allocation"] = sec_current["value"] / total_value * 100

    # Compute "To Contrib" = amount needed to reach target weight, never negative
    sec_current["to_contrib"] = np.maximum(((sec_current["target_pct"] - sec_current["allocation"]) / 100) * total_value, 0)

    # Format columns for display
    # (Create new display columns to avoid breaking numeric cols used below)
    sec_current["allocation_disp"] = sec_current["allocation"].map(lambda x: f"+{x:.2f}%" if x >= 0 else f"{x:.2f}%")
    sec_current["target_pct_disp"] = sec_current["target_pct"].map(lambda x: f"+{x:.2f}%" if x >= 0 else f"{x:.2f}%")
    sec_current["to_contrib_disp"] = sec_current["to_contrib"].map(lambda x: f"${x:,.2f}")

    # Build table rows
    table_rows = []
    for _, row in sec_current.iterrows():
        table_rows.append([
            row["ticker"],
    ...
    # Add TOTAL row
    total_value_sum = sec_current["value"].sum()
    total_to_contrib_sum = sec_current["to_contrib"].sum()
    table_rows.append([
        "TOTAL",
        "",
        "",
    ...
    # Footer
    p = doc.add_paragraph("Delta % = actual allocation minus target allocation")
    format_footer(p)
    ...
    # Merge actual values from sec_current with target_pct
    ticker_merge = sec_current[["ticker", "value"]].merge(
        holdings_df[["ticker", "target_pct"]],
        on="ticker",
        how="left"
    )
    ...
    p = doc.add_paragraph("Actual vs target allocation by ticker.", style="Normal")
    ...
    # Merge actual values from sec_current
    sec_only_grouped = (
        sec_current.groupby("asset_class_short")
        .agg(value=("value", "sum"))
        .reset_index()
    )
    ...
    p = doc.add_paragraph(
        "Figure: Portfolio returns use true TWR based on a flow-adjusted, end-of-day PV series (external flows treated at start-of-day). Benchmark returns use total return (adjusted close) and are rebased to the same start date for comparability.",
        style="Normal"
    )
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    format_footer(p)
    ...
    p = doc.add_paragraph(
        ...
        "Benchmarks use total returns (adjusted close) rebased to the same since-inception start date.",
        style="Normal"
    )
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    format_footer(p)
    ...
    p = doc.add_paragraph(
        "Portfolio % uses flow-adjusted TWR. Benchmark % uses total returns (adjusted close) "
    ...
    if class_full.empty and sec_full.empty:
        ...
        for ac in asset_classes:
            ...
            # Filter to show only current holdings (shares > 0)
            tickers_in_ac = (
                sec_full[
                    (sec_full["asset_class"] == ac) & 
                    (sec_full["shares"].abs() > 1e-6)
                ]
    ...
    if class_full.empty and sec_full.empty:
        ...
        for ac in asset_classes_pl:
            ...
            for h in horizons_pl:
                # Use centralized engine logic (replaces local compute_asset_class_pl)
                val = calculate_asset_class_pl(
                    ac, h, prices, pv, inception_date, tx_all_raw, sec_only, div_all_raw
                )
                if val is None:
                    row_vals.append("N/A")
                else:
                    row_vals.append(fmt_dollar_clean(val))
            rows_pl.append(row_vals)

            # ----- Ticker rows under that class -----
            # Filter to show only current holdings (shares > 0)
            tickers_in_ac = (
                sec_current[sec_current["asset_class"] == ac]
                .sort_values("ticker")["ticker"]
                .tolist()
            )
        ...
        # -------------------------------------------------------
        # CASH P/L ROW - GIPS COMPLIANT
        # For proper reconciliation: Sum(Ticker P/Ls) + Cash P/L = Portfolio P/L
        # Cash earns 0% return, so Cash P/L = 0 for all horizons
        # -------------------------------------------------------
        cash_row = ["Cash / Recon"]
        ...
        for h in horizons_pl:
            # Determine horizon start (same as portfolio horizon)
            if h == "SI":
                raw_start = inception_date
            else:
                raw_start = get_portfolio_horizon_start(pv, inception_date, h)
            ...
            # Sum Ticker P/Ls using PV start alignment (first trading day)
            sum_ticker_pl = 0.0
            all_tickers = sec_only[sec_only["ticker"] != "CASH"]["ticker"].unique()
            
            # FIX: Use pv.index.min() (first trading day) instead of inception_date
            ...
            # Cash P/L = Portfolio P/L - Sum(Ticker P/Ls)
            # This should be ~0 when properly aligned (cash earns no return)
            cash_pl = port_pl - sum_ticker_pl
            cash_row.append(fmt_dollar_clean(cash_pl))
        ...
        # Consolidated Footnote for Returns Table
        ret_footer_text = (
            "Note: Returns computed using Modified Dietz methodology. Returns require full measurement period per GIPS standards "
            "(e.g., 1M return requires 30+ days of position history). N/A indicates insufficient data for that horizon."
        )
        p = doc.add_paragraph(ret_footer_text, style="Normal")
        format_footer(p)
        
        # Consolidated Footnote for P/L Table
        pl_footer_text = (
            "Note: Economic P/L (MV End - MV Start - Net Flows + Dividends) may be displayed for positions with insufficient history for a valid return %. "
            "This is GIPS-compliant: returns require full-period data, but P/L reflects actual economic outcomes regardless of holding period."
        )
        p = doc.add_paragraph(pl_footer_text, style="Normal")
        format_footer(p)
    ...
    # Use full price history for robust volatility
    dynamic_risk_return, _ = _calculate_dynamic_risk_profile(prices, sec_current, holdings_df)

    # ===========================
    # Expected Volatility By Asset Class (Bar Chart)
    # ===========================
    doc.add_heading("Expected Volatility by Asset Class (Realized)", level=2)

    # Use keys from dynamic calc
    # Filter to show only a subset of key classes if desired, or all.
    
    # Original keys from the report (if available):
    target_keys = [
        "Digital Assets", "Gold", "International Equity", 
        "US Bonds", "US Growth", "US Large Cap", "US Small Cap"
    ]
    
    classes = []
    vol_vals = []
    
    # Use all available keys from dynamic calc that match target list, plus others?
    # Let's prioritize target list, then add others.
    available_keys = list(dynamic_risk_return.keys())
    
    # First add targets if they exist
    for k in target_keys:
        if k in dynamic_risk_return:
            classes.append(k)
            vol_vals.append(dynamic_risk_return[k]["vol"])
            
    # Add any others not in target list
    ...
    caption = doc.add_paragraph("Figure: Annualized realized volatility (10Y).", style="Normal")
    caption.alignment = WD_ALIGN_PARAGRAPH.CENTER
    format_footer(caption)
    ...
    # ===========================
    doc.add_heading("Risk vs Expected Return (Realized)", level=2)
    ...
    for k in classes:
        if k in dynamic_risk_return:
            vols.append(dynamic_risk_return[k]["vol"])
            rets.append(dynamic_risk_return[k]["return"])
            labels.append(k)
    ...
    # Dynamic fetcher handles any ticker
    sector_universe = sec_current[sec_current["ticker"] != "CASH"].copy()
    ...
    run.add_picture(img_stream, width=Inches(6))
    paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p = doc.add_paragraph("Figure: Approximate sector exposure for portfolio ETFs.", style="Normal")
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    format_footer(p)
    ...
    p = doc.add_paragraph()
    r = p.add_run("Data Sources. ")
    r.bold = True
    p.add_run(
        "Sector classifications are provided by Financial Modeling Prep (FMP). "
        "Historical price data is provided by Yahoo Finance. "
    ...
    r.bold = True
    p.add_run(
        "Benchmark figures (such as broad equity or balanced indices) are shown as total returns (adjusted close). "
