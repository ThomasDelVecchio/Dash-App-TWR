...
def _plot_pie_with_smart_labels(ax, labels, values, title):
    ...
    for i, p in enumerate(wedges):
        ...
        if percentage < 4.0:
            ...
            ax.annotate(f"{labels[i]} ({percentage:.2f}%)", xy=(x, y), xytext=(1.35*np.sign(x), 1.4*y),
...
def plot_ticker_allocation_pie(ticker_labels, ticker_values):
    ...
    _plot_pie_with_smart_labels(ax, ticker_labels, ticker_values, "Ticker Allocation")
...
def plot_ticker_allocation_bar(ticker_merge):
    ...
    for i in range(len(ticker_merge)):
        ax.text(i - width/2, ticker_merge["actual_pct"].iloc[i] + 0.5,
                f"{ticker_merge['actual_pct'].iloc[i]:.2f}%", ha='center', va='bottom', fontsize=8)
        ax.text(i + width/2, ticker_merge["target_pct"].iloc[i] + 0.5,
                f"{ticker_merge['target_pct'].iloc[i]:.2f}%", ha='center', va='bottom', fontsize=8)
...
def plot_asset_allocation_pie(labels, values):
    fig, ax = plt.subplots(figsize=(6, 6))
    _plot_pie_with_smart_labels(ax, labels, values, "Asset Allocation")
...
def plot_asset_allocation_bar(ticker_merge):
    ...
    for i in x:
        ax.text(i, ticker_merge["actual_pct"].iloc[i]+0.5,
                f"{ticker_merge['actual_pct'].iloc[i]:.2f}%", ha="center", va="bottom", fontsize=9)
        ax.text(i + width, ticker_merge["target_pct"].iloc[i]+0.5,
                f"{ticker_merge['target_pct'].iloc[i]:.2f}%", ha="center", va="bottom", fontsize=9)
