...
import config
from config import TARGET_MONTHLY_CONTRIBUTION, GLOBAL_PALETTE, RISK_FREE_RATE, TAX_RATE_LT, TAX_RATE_ST, BENCHMARK_PRESETS
...
# ============================================================
# STRATEGY BACKTESTING (STRATEGY ARENA)
# ============================================================
...
def _apply_lookback_window(prices: pd.DataFrame, lookback: str, common_start: pd.Timestamp) -> pd.DataFrame:
    if prices.empty:
        return prices

    lookback_map = {"1Y": 1, "3Y": 3, "5Y": 5}
    end_date = prices.index.max()

    if lookback in lookback_map:
        lb_start = end_date - pd.DateOffset(years=lookback_map[lookback])
        start_date = max(common_start, lb_start)
    else:
...
def get_strategy_backtest_results(
    ...
    prices = fetch_price_history(all_tickers, use_adj_close=True)
    if prices.empty:
        return {"error": "Price history unavailable for selected strategies."}

    # Ensure we only keep requested tickers (fetch_price_history may add SPY)
    prices = prices[[c for c in prices.columns if c in all_tickers]]

    common_start = _get_common_start_date(prices, all_tickers)
    if common_start is None:
        return {"error": "Insufficient price history for selected tickers."}

    prices = _apply_lookback_window(prices, lookback, common_start)
    if prices.empty:
    ...
    for strat in strategies:
        ...
        strat_returns = returns[tickers].copy()
